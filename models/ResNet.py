...
#         if self.conf.use_resnet18 and self.conf.use_pad_for_resnet18_Bottleneck3D:
...
#         self.hist = HistYusufLayer(self.conf.centers, self.conf.widths)
#         self.hist = HistYusufLayer(n_bins=self.conf.nbins, inchannel=self.conf.last_feature_dim, centers=self.conf.centers, width=self.conf.widths)
        ...
        if self.conf.use_linear_to_merge_features and self.conf.use_hist:
            if not self.conf.use_just_last_bin:
                in_channel = 1 + self.hist.nbins if self.conf.concat_hist_max else self.hist.nbins
            else:
                in_channel = 1 + 1 if self.conf.concat_hist_max else 1

            self.linear_merge_features = nn.Linear(in_channel, 1)
        
        if self.conf.use_dropout:
        ...
        if self.conf.use_hist:
            if self.conf.use_linear_to_merge_features or self.conf.use_hist_and_max_seprately:
                ...
                _coef = (self.hist.nbins + 1) if self.conf.concat_hist_max else (self.hist.nbins)

            if self.conf.use_linear_to_get_important_features:
                self.feature_reduction = nn.Sequential(
                    nn.Linear(self.conf.last_feature_dim * (_coef), self.conf.last_feature_dim),
                    # nn.BatchNorm1d(self.conf.last_feature_dim),
                    # nn.ReLU()
                )
                # if this is true then _coef is must be one but we need the value later so we don't assinged it to 1
                self.bn = nn.BatchNorm1d(self.conf.last_feature_dim)
                self.classifier = nn.Linear(self.conf.last_feature_dim, num_classes)
            else:
                self.bn = nn.BatchNorm1d(self.conf.last_feature_dim * _coef)
                self.classifier = nn.Linear(self.conf.last_feature_dim * (_coef), num_classes)
        else:
            self.bn = nn.BatchNorm1d(self.conf.last_feature_dim)
            self.classifier = nn.Linear(self.conf.last_feature_dim, num_classes)
